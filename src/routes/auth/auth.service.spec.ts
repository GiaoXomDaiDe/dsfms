...
describe('AuthService', () => {
  ...
  describe('validateUser', () => {
    ...
    it('should throw MissingCredentialsException when email is missing', async () => {
      await expect(service.validateUser({ email: '', password: 'password' }))
        .rejects.toThrow(AuthErrors.MissingCredentialsException);
    });

    it('should throw MissingCredentialsException when password is missing', async () => {
      await expect(service.validateUser({ email: 'test@example.com', password: '' }))
        .rejects.toThrow(AuthErrors.MissingCredentialsException);
    });
    ...
    it('should throw UserNotFoundException when user does not exist', async () => {
      ...
      await expect(service.validateUser(authPayload))
        .rejects.toThrow(AuthErrors.UserNotFoundException);
    });
    ...
    it('should throw AccountDisabledException when user is disabled', async () => {
      ...
      await expect(service.validateUser(authPayload))
        .rejects.toThrow(AuthErrors.AccountDisabledException);
    });
    ...
    it('should throw InvalidCredentialsException when password is invalid', async () => {
      ...
      await expect(service.validateUser(authPayload))
        .rejects.toThrow(AuthErrors.InvalidCredentialsException);
    });
    ...
    it('should throw AuthenticationServiceException on unexpected error', async () => {
      ...
      await expect(service.validateUser(authPayload))
        .rejects.toThrow(AuthErrors.AuthenticationServiceException);
    });
  });
  ...
  describe('refreshTokens', () => {
    ...
    it('should throw TokenExpiredException when token is expired', async () => {
      ...
      await expect(service.refreshTokens(refreshToken))
        .rejects.toThrow(AuthErrors.TokenExpiredException);
    });
    ...
    it('should throw InvalidRefreshTokenException for invalid token', async () => {
      ...
      await expect(service.refreshTokens(refreshToken))
        .rejects.toThrow(AuthErrors.InvalidRefreshTokenException);
    });
  });
  ...
  describe('forgotPassword', () => {
    ...
    it('should throw MissingCredentialsException when email is missing', async () => {
      await expect(service.forgotPassword('', magicLink))
        .rejects.toThrow(AuthErrors.MissingCredentialsException);
    });
  ...
  describe('resetPassword', () => {
    ...
    it('should throw MissingCredentialsException when token is missing', async () => {
      await expect(service.resetPassword('', newPassword))
        .rejects.toThrow(AuthErrors.MissingCredentialsException);
    });

    it('should throw MissingCredentialsException when password is missing', async () => {
      await expect(service.resetPassword(token, ''))
        .rejects.toThrow(AuthErrors.MissingCredentialsException);
    });
    ...
    it('should throw InvalidTokenException for invalid token type', async () => {
      ...
      await expect(service.resetPassword(token, newPassword))
        .rejects.toThrow(AuthErrors.InvalidTokenException);
    });
