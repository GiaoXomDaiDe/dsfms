...
export class TemplateSectionNotFoundError extends NotFoundException {
  constructor(sectionId?: string) {
    super(sectionId ? `Template section with ID '${sectionId}' not found` : 'Template section not found')
  }
}
...
export class TemplateFieldNotFoundError extends NotFoundException {
  constructor(fieldId?: string) {
    super(fieldId ? `Template field with ID '${fieldId}' not found` : 'Template field not found')
  }
}
...
export class DocxParsingError extends BadRequestException {
  constructor(details?: string) {
    super(details ? `Failed to parse DOCX template: ${details}` : 'Failed to parse DOCX template')
  }
}

export class InvalidDocxTemplateError extends BadRequestException {
  constructor(reason?: string) {
    super(reason ? `Invalid DOCX template: ${reason}` : 'Invalid DOCX template format')
  }
}
...
export class TemplateCreationFailedError extends BadRequestException {
  constructor(reason?: string) {
    super(reason ? `Failed to create template: ${reason}` : 'Failed to create template')
  }
}

export class TemplateUpdateFailedError extends BadRequestException {
  constructor(reason?: string) {
    super(reason ? `Failed to update template: ${reason}` : 'Failed to update template')
  }
}

export class TemplateDeletionFailedError extends BadRequestException {
  constructor(reason?: string) {
    super(reason ? `Failed to delete template: ${reason}` : 'Failed to delete template')
  }
}
...
export class InvalidTemplateSchemaError extends BadRequestException {
  constructor(reason?: string) {
    super(reason ? `Invalid template schema: ${reason}` : 'Invalid template schema format')
  }
}

export class TemplateSchemaGenerationError extends BadRequestException {
  constructor(reason?: string) {
    super(reason ? `Failed to generate template schema: ${reason}` : 'Failed to generate template schema')
  }
}
...
export class TransactionFailedError extends BadRequestException {
  constructor(reason?: string) {
    super(reason ? `Transaction failed: ${reason}` : 'Database transaction failed')
  }
}
